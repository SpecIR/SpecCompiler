...
--   4. create_verification_views execs each entry's SQL
...
return function(actual_doc, helpers)  -- luacheck: ignore actual_doc helpers
    ...
    local vs = host:get_verification_views()
    ...
    vs = host:get_verification_views()
    ...
    -- 4. create_verification_views execs each entry's SQL
    host:create_verification_views(mock)
    local found_b2 = false
    for _, s in ipairs(exec_sqls) do
        if tostring(s):find("v_b2", 1, true) then found_b2 = true end
    end
    if not found_b2 then err("create_verification_views did not exec the v_b2 SQL") end

    -- 5. a non-message hook on the verification kind errors loudly
    if pcall(function()
        host:register{ kind = "verification", schema = { id = "pk_x", policy_key = "pk_x" },
