-- Test oracle for VC-REL-007: dictionary cross-references in sw_docs.
--
-- XREF_DIC is a LABEL_REF (# selector). Therefore:
--   * a @-PID link to a dictionary entry resolves as the GENERIC section
--     cross-reference XREF_SEC (DIC extends TRACEABLE -> SECTION); and
--   * a #-label link resolves to the type-specific XREF_DIC.
-- The distance-aware scorer keeps the #-label case unambiguous: XREF_DIC's
-- exact DIC target (distance 0) beats XREF_SECP's SECTION (distance > 0).
...
return function(actual_doc, helpers)
    ...
    local cases = {
        { "DIC-AUTH-001",       "@", "XREF_SEC", "DIC" },
        { "DIC-TOKEN-001",      "@", "XREF_SEC", "DIC" },
        { "dic:authentication", "#", "XREF_DIC", "DIC" },
        { "dic:session-token",  "#", "XREF_DIC", "DIC" },
    }
    ...
    for _, case in ipairs(cases) do
        ...
        local r = rel(target)
        if not r then
            err("Missing relation HLR-DIC-001 -> " .. target)
        else
